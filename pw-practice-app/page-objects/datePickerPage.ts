...
export class DatePickerPage extends HelperBase {
    ...
    async selectDatePickerWithRangeFromToday(startDayFromToday: number, endDauFromToday: number) {
        const calendarInputField = this.page.getByPlaceholder('Range Picker');
        await calendarInputField.click();

        const dateToAssertStart = await this.selectDateInCalendar(startDayFromToday);
        const dateToAssertEnd = await this.selectDateInCalendar(endDauFromToday);
